...
## 2. 데모 영상
[데모 시연 영상](여기에_유튜브_또는_드라이브_링크_삽입)
...
* 양육자(부/모)가 각자의 스마트폰에서 접근하더라도 완벽하게 동일한 Document를 바라보게 만들어 한쪽에서 버튼을 누르면 다른 스마트폰과 교사 대시보드에 딜레이 없이 100% 양방향 동기화 가능  

### 대규모 트래픽 렌더링 부하 방어
* 문제: 하원 시간에 트래픽이 몰릴 때마다 DB를 계속 새로 긁어오면 과도한 읽기 비용 및 UI 버벅임 발생.
